...
const builderConfig = require("./build/build");
...
builder.usePlugin([
  ...
  new CopyWebpackPlugin(option.assets)
]);
...
option.template.enable &&
  builder.useHtmlPlugin(
    devPath.template,
    option.SP.enable
      ? [...option.SP.scrpts, ...option.template.scripts]
      : option.template.scripts,
    option.template.css
  );
