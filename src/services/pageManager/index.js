...
PageManager.prototype.setDatas = function(items, indexPage = 0, pageSize = 0) {
  if (Array.isArray(this.ManagerInfo.items)) {
    ...
    this.ManagerInfo.indexPage = indexPage
      ? indexPage
      : this.ManagerInfo.indexPage;
...
PageManager.prototype.getData = function(index) {
  index =
    index - (this.ManagerInfo.indexPage - 1) * this.ManagerInfo.pageSize - 1;
 ...
 */
PageManager.prototype.redirectIndex = function(
  pageNum,
  hasNext,
  items,
  pagingInfo = ""
  ...
  if (pagingInfo) {
    this.ManagerInfo.pagingInfos[
      index + this.ManagerInfo.displayPageNum
    ] = pagingInfo;
  }
};
 ...
 * @param {string} pagingInfo 存放下一次翻页的定位信息
 */
PageManager.prototype.redirectIndexDB = function(
  pageNum,
  items,
  count,
  pagingInfo = ""
...
PageManager.prototype.getPagingInfo = function(pageNum) {
  let index = PageManagerInfo.calcIndex(
    pageNum,
    this.ManagerInfo.displayPageNum
  );
  return this.ManagerInfo.pagingInfos[index]
    ? this.ManagerInfo.pagingInfos[index]
    : "";
};
